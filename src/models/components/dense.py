...
class DenseNet(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        """
        super().__init__()
        self.linear_RoBERTa = LinearLayer(769*6, 600)
        self.linear_client_1 = LinearLayer(674*6, 600)
        self.linear_counselor_1 = LinearLayer(674*6, 600)
    ...
    def forward(self, x1, x2, x3: torch.Tensor) -> torch.Tensor:
        ...
        x_RoBERTa = self.linear_RoBERTa(x_RoBERTa.flatten(start_dim=1))
        x_client = self.linear_client_1(x_client.flatten(start_dim=1))
        x_counselor = self.linear_counselor_1(x_counselor.flatten(start_dim=1))
        # linear layers
